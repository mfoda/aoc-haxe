...
class D04B {
	...
	static function parsePassports():Array<Passport> {
		...
		for (chunk in data) {
			var fields = ~/\s/g.split(chunk);
			passports.push(mkPassport(fields));
		}

		return passports;
	}

	static function mkPassport(fields:Array<String>):Passport {
		var p:Passport = { byr: null, iyr: null, eyr: null, hgt: null, hcl: null, ecl: null, pid: null, cid: null };

		for (field in fields) {
			var _field = field.split(":");
			var name = _field[0], value = _field[1];
			switch (name) {
				case "byr":
					p.byr = Std.parseInt(value);
				case "iyr":
					p.iyr = Std.parseInt(value);
				case "eyr":
					p.eyr = Std.parseInt(value);
				case "hgt":
					p.hgt = value;
				case "hcl":
					p.hcl = value;
	...
	static function isValidPassport(p:Passport):Bool {
		function valid_byr() 
		...
		function valid_hgt()
			return {
				var hgt = Std.parseInt(p.hgt.substring(0, p.hgt.length - 2));
		...
		function valid_hcl()
			return ~/^#[a-z0-9]{6}$/.match(p.hcl);
		function valid_ecl()
			return ~/(amb|blu|brn|gry|grn|hzl|oth)/.match(p.ecl);
		function valid_pid()
			return ~/^[0-9]{9}$/.match(p.pid);

		return (
			p.byr != null && valid_byr() &&
			p.iyr != null && valid_iyr() &&
			p.eyr != null && valid_eyr() &&
			p.hgt != null && valid_hgt() &&
			p.hcl != null && valid_hcl() &&
			p.ecl != null && valid_ecl() &&
			p.pid != null && valid_pid() 
		);
	}

	static inline var input = "byr:1937
