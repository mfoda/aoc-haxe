...
class D02B {
	...
	static function isValidPassword(entry:String):Bool {
		var isValid = false;

		var m = ~/(\d+)-(\d+) (\w): (\w+)/;
		if (m.match(entry)) {
			var idx1 = Std.parseInt(m.matched(1)),
				idx2 = Std.parseInt(m.matched(2)),
				target = m.matched(3),
				password = m.matched(4),
				occurrence1 = password.charAt(idx1 - 1) == target,
				occurrence2 = password.charAt(idx2 - 1) == target;

			isValid = (occurrence1 != occurrence2);
		}

		return isValid;
	}

	static inline var input = "9-10 b: bbktbbbxhfbpb
2-10 x: xxnxxxwxxsx
5-7 w: ghwwdrr
4-6 z: nzzjzk
7-8 s: szsssswfs
