...
class D06B {
	...
	static function scoreGroup(answers:String):Int {
		...
		var allAnswers = answers.split("\n").map(getPersonAnswers);

		for (i in 0...questions.length) {
			var answeredByAll = true;
			for (answer in allAnswers) {
				if (!answer.exists(questions.charAt(i))) {
					answeredByAll = false;
					break;
				}
			}
	...
	static function getPersonAnswers(answer:String):Map<String, Bool> {
		var answers = new Map<String, Bool>();
		for (i in 0...answer.length) {
			answers[answer.charAt(i)] = true;
		}
		return answers;
	}

	static inline var input = "qpicundo
fiqcdbkyuoz
