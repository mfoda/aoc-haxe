...
class D06B {
	...
	static function process() {
		...
		for (ins in instructions)
			for (row in ins.y1...ins.y2 + 1)
				for (col in ins.x1...ins.x2 + 1)
					lights[row][col] = ins.action(lights[row][col]);

		function sum(a, b)
			return a + b;
		var sumBrightness = lights.map(row -> row.fold(sum, 0)).fold(sum, 0);

		Sys.println('The total brightness is off all lights is $sumBrightness');
	}

	static function parseInstruction(str:String):Instruction {
		var patt = ~/(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)/;
		if (patt.match(str)) {
			var m = patt.matched,
				x1 = Std.parseInt(m(2)),
				y1 = Std.parseInt(m(3)),
				...
				act = switch (m(1)) {
					case "turn on": x -> x + 1;
					case "turn off": x -> Std.int(Math.max(0, x - 1));
					case "toggle": x -> x + 2;
		...
		throw "can't parse instruction";
	}

	static inline var input = "toggle 461,550 through 564,900
